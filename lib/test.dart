...
import 'package:homepage/home/appointment.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  final List<Widget> _pageOptions = [Shop(), Appointment(), Alternative()];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: StreamBuilder(
        stream: Firestore.instance.collection('user_details').snapshots(),
        builder: (context,snapshot) {
          if(!snapshot.hasData){
            ...
            while(i<snapshot.data.documents.length){
              if(snapshot.data.documents[i].documentID.toString() == uid){
                j=i;
                break;
              }
              i++;
            };
            DocumentSnapshot mydetails = snapshot.data.documents[j];
            //while(i<=snapshot.data.documents.length)
//            for(int i=0; i<snapshot.data.documents.length;i++){
//              if(snapshot.data.documents[i].documentID.toString() == getCurrentUser().toString()){
//                print("Hello");
//                break;
//              }
//            }
              ...
              child: new ListView(
                children: <Widget>[
                  UserAccountsDrawerHeader(
                    accountName: Text('${mydetails['name']}'),
                    accountEmail: Text('${mydetails['email']}'),
